...
<div class="app">
    <div class="login-frame">
        ...
        <div class="input-part">
            <button class="btn btn-primary input-btn" @click="register()">注册</button>
        </div>
        <div class="input-part register">
            <a href="<c:url value="/user/login/index"/>">我有账号，立即登录</a>
        </div>
    </div>
...
<script>
    ...
    var vm = new Vue({
        ...
        methods:{
            register:function(){
                ...
                $.post("<c:url value="/user/register"/>",
                    {
            ...
            annalysData:function(_data) {
                if(!_data){
                    ...
                    if(_data.refreshx == "true"){
                        init();
                        vm.modalMessage = '你和服务器失去了联系，请再次点击注册！';
                    }
    ...
    function init() {
        $.get("<c:url value="/user/register/randomsalt"/>",function (data, status) {
            if (status) {
                vm.salt = eval(data)['salt'];
            }
        });
